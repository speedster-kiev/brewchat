...
- Ingredient matching against a cached copy of the supplier's catalog via a `search_catalog` tool.
  - Acceptance: given a parsed ingredient, the tool returns the best-matching candidates (with availability, price, link) or explicitly returns "no match."
...
- Final output: a formatted list with matched/substituted items, quantities, unit prices, and a total, assembled via a `build_order_list` tool that turns each product's `Handle` into a full product URL using the base URL from config.
  ...
  - Acceptance (issue #2): quantities and prices respect the pack size in the product title. When the title states a weight or volume ("pr. 100 g.", "pr. 25 kg.", "100 g", "1 liter"), `build_order_list` computes the quantity in code as the recipe amount divided by the pack size, rounded up (5 kg of a malt priced per 100 g is 50 units; 20 kg of a malt sold in 25 kg sacks is 1). The agent's own quantity is used only when the units can't be compared (yeast "1 pack" against an 11,5 g sachet, or a title with no size). Each such line shows what is bought ("buying 50 x 100 g = 5 kg"), and any quantity the code corrected is reported back to the agent.
...
[Backend: Claude agent loop, via Tool Runner, system prompt = role + scope + substitution rules]
        ...
        |     availability, price, handle. Called once per ingredient, and
...
`quantity` is the number of catalog units to buy. One unit is whatever the title's pack size says (100 g, a 25 kg sack, a 100 g hop pack) or one pack when the title has no size. For weight and volume pack sizes the tool recomputes it from `ingredient.amount` and `unit` (see the `build_order_list` acceptance criteria), so when the agent merges repeated additions of one product it puts the summed amount in `ingredient.amount`.
