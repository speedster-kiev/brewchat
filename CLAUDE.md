...
- `system-prompt.md` is the runtime source of the live prompt (`agent/prompt.py` reads it at runtime). Before editing it, load the `system-prompt-edit` skill — it covers the placeholder rules and the required adversarial-eval re-run.
...
- Pack size, origin (issue #1), beer style ranking, and the in-stock definition are all non-obvious domain rules spread across `catalog/pack.py`, `catalog/origin.py`, and `catalog/search.py`. Before touching any of them, load the `catalog-domain-rules` skill.
- `Handle` is already a full path, so link = `base_url.rstrip('/') + handle`; `Prices` is a single-element list.
