...
uv run scripts/check_origins.py                      # origin coverage, producers missing from [origins]
...
- Origin (issue #1) is derived, not stored: `catalog/origin.py` maps a title to an ISO code (producer map from the `[origins]` config first, then a standalone country token such as `UK`/`CZ`; `ES` is deliberately not read as Spain) and splits origin words ("Belgian", "UK") out of a query. `CatalogIndex.search` matches the text without them and uses the origin only as a tie-break tier among equal scores, so it can never promote a worse match. `origin: null` means unknown. `uv run scripts/check_origins.py` lists producers with no mapping.
- Beer style ranks, it never filters: `submit_parsed_recipe(style=...)` is kept on `Session.style`, `search_catalog` passes it to `CatalogIndex.search`, and the `[style_origins]` config (style → usual origin codes; `style_origins()` in `origin.py`) adds a tie-break tier after the query's own origin. Candidates carry `suggested` (best in-stock among the top scorers) and `why`. The prompt tells the agent to build with the suggestion and list the equally good alternatives after the list; that reply behaviour is covered by eval D6, not by unit tests.
